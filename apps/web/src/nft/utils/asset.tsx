...
export const generateTweetForAsset = (asset: GenieAsset): string => {
  ...
  }%20(${asset.collectionName})%20${REACT_APP_HOST_NAME}/nfts/asset/${asset.address}/${asset.tokenId}%20via%20@uniswap`
}
...
export const generateTweetForPurchase = (assets: UpdatedGenieAsset[], txHashUrl: string): string => {
  ...
  } with @Uniswap 🦄\n\n${REACT_APP_HOST_NAME}/nfts/${collectionUrl}\n${txHashUrl}`
  ...
  const tweetText =
    assets.length == 1
      ? `I just listed ${
          ...
          .join(', ')}. Buy it on @Uniswap at ${REACT_APP_HOST_NAME}/${getAssetHref(assets[0])}`
      : `I just listed ${
          assets.length
        } items on @Uniswap at ${REACT_APP_HOST_NAME}/nfts/profile\n\nCollections: ${mapAssetsToCollections(assets)
